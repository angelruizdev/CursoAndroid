...
public class FragmentSqliteRecycler extends Fragment implements View.OnClickListener {
    View vista;
    SQLiteOpnHpr conn;//instanciamos la conexion a la bd
    Context context;
    String precioProducto,nombreProducto;
    EditText etPrecioProducto, etNomProducto, etBuscIdProducto, etCoinsidencia, etCoinsidencia1, etElimIdProducto;
    Button btGuardarProducto, btBuscarProducto, btActualizarProducto, btEliminarProducto;
    ...
    public void guardarDatos(){
      ...
      if (!etPrecioProducto.getText().toString().equals("") && !etNomProducto.getText().toString().equals("")) {
          ...
          SQLiteDatabase db = conn.getWritableDatabase();//abrimos la bd para escribir(guardar) en ella, mediante el objeto conn
    ...
    public void buscarProducto(){
        SQLiteDatabase db = conn.getReadableDatabase();//para mostrar registro se obtiene el metodo de lectura de bd mediante la conexion(conn)
